...
import { Briefcase, Building2, Clock, PlaneTakeoff, Repeat, Route as RouteIcon } from "lucide-react";
...
const content = routeContent.routeAthensAirportAthenaeumIC.el;
const { meta } = content;

export const metadata: Metadata = {
  title: { absolute: meta.title },
  description: meta.description,
  alternates: localizedAlternates("routeAthensAirportAthenaeumIC", "el"),
  openGraph: { title: meta.title, description: meta.description, url: absoluteUrl(pathFor("routeAthensAirportAthenaeumIC", "el")) },
  twitter: { title: meta.title, description: meta.description },
};

export default function AthensAirportAthenaeumICRoutePageEl() {
  ...
  return (
    ...
    >
      ...
      <ServiceFeatures content={content.whoItSuits} icons={[Briefcase, Building2, Building2]} />
      <RelatedLinks eyebrow={content.relatedLinks.eyebrow} title={content.relatedLinks.title} items={content.relatedLinks.items.map((item, i) => ({ ...item, icon: [PlaneTakeoff, Briefcase][i] }))} />
