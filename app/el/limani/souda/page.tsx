...
const content = portContent.portSouda.el;
const { meta } = content;

export const metadata: Metadata = {
  title: { absolute: meta.title },
  description: meta.description,
  alternates: localizedAlternates("portSouda", "el"),
  openGraph: { title: meta.title, description: meta.description, url: absoluteUrl(pathFor("portSouda", "el")) },
  twitter: { title: meta.title, description: meta.description },
};

export default function SoudaPortPageEl() {
  ...
  return (
    ...
    >
      ...
      <RelatedLinks eyebrow={content.connections.eyebrow} title={content.connections.title} items={content.connections.items.map((item) => ({ ...item, icon: Building2 }))} />
      <ServiceOverview content={content.cruiseFerryContext} />
      <RelatedLinks eyebrow={content.popularDestinations.eyebrow} title={content.popularDestinations.title} items={content.popularDestinations.items.map((item) => ({ ...item, icon: Route }))} />
