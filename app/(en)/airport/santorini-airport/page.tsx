...
const content = airportContent.santoriniAirport.en;
const image = airportImages.santoriniAirport;
const { meta } = content;

export const metadata: Metadata = {
  title: { absolute: meta.title },
  description: meta.description,
  alternates: localizedAlternates("santoriniAirport", "en"),
  openGraph: { title: meta.title, description: meta.description, url: absoluteUrl(pathFor("santoriniAirport", "en")) },
  twitter: { title: meta.title, description: meta.description },
};

export default function SantoriniAirportPage() {
  const quoteHref = pathFor("getQuote", "en");

  return (
    <LocationPage
      pageKey="santoriniAirport"
      locale="en"
      title={meta.title}
      description={meta.description}
      breadcrumbItems={[
        { label: "Home", href: "/" },
        { label: "Airports", href: pathFor("destinationsHub", "en") },
    ...
    >
      ...
      <RelatedLinks eyebrow={content.destinations.eyebrow} title={content.destinations.title} items={content.destinations.items.map((item, i) => ({ ...item, icon: [Landmark][i] }))} />
      <RelatedLinks eyebrow={content.connections.eyebrow} title={content.connections.title} items={content.connections.items.map((item, i) => ({ ...item, icon: [Car, Users][i] }))} />
      <ServiceHowItWorks content={content.howPickupWorks} />
      <ServiceFeatures content={content.travelPlanning} icons={[CalendarClock, PlaneTakeoff, MessageCircle, Landmark]} />
